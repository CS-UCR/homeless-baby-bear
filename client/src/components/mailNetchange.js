...
export class Netchange extends Component {
        ...
        return (
            <div className='card'>
                ...
                <span className='net-change-percent'>
                    <span className='sign positive'>
                        {' '}
                        {this.calculateNetChange().percentage}{' '}
                    </span>
                </span>
                <span className='net-change-value'>
                    <span id='net-change-msg'>
                        ...
                        mail recieved vs.
                        <span id='time-choice'>{this.state.chart_choice}</span>
                    </span>
                </span>
            </div>
        );
    ...
    calculateNetChange = () => {
        return {
            percentage:
                this.props.state['last_' + this.state.chart_choice + '_num'] !==
                0
                    ? (this.props.state[this.state.chart_choice + '_num'] >=
                      this.props.state[
                          'last_' + this.state.chart_choice + '_num'
                      ]
                          ? '+' +
                            parseInt(
                                parseInt(
                                    this.props.state[
                                        this.state.chart_choice + '_num'
                                    ]
                                ) /
                                    parseInt(
                                        this.props.state[
