...
export class Chart extends Component {
    ...
    render() {
        ...
        if (this.props.chartTimeframe === 'week') {
            chart = (
                <Bar data={this.getLastWeekData} options={this.getOptions()} />
            );
            ...
            chart = (
                <Line data={this.getLastYearData} options={this.getOptions()} />
            );
        } else if (this.props.chartTimeframe === 'lifetime') {
            chart = (
                <Line data={this.getLifetimeData} options={this.getOptions()} />
            );
        }
    ...
    getOptions = () => {
        return {
            ...
            title: {
                display: true,
                text:
                    'Total mail sent this past ' +
                    this.props.chartTimeframe +
                    '. Total: ' +
                    this.props.state[this.props.chartTimeframe].reduce(
                        (a, b) => a + b,
                        0
                    ),
                fontSize: 18,
