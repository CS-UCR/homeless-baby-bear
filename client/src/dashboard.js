...
class SimpleMap extends Component {
    ...
    getDataFromDbDate = (fromDate, toDate, var_name, i) => {
        axios
            .post(process.env.REACT_APP_API + '/getData_bydate', {
                fromDate: fromDate,
                toDate: toDate,
                location_type: 'ALL',
            })
            .then(res => {
                ...
                if (var_name === this.state.lifetime)
                    this.setState({
                        allData: this.state.allData.concat(res.data.data),
                    });
            });
    };
            ...
            if (this.state.getLifetime === false && i !== numLabels - 2) {
                this.getDataFromDbDate(
                    ...
                    new Date(
                        ...
                        startDate.getDate() +
                            1 +
                            Math.ceil(dayDifference / numLabels - 1) * (i + 1)
                    ),
                    this.state.lifetime,
                    i
                );
    ...
    setValue = () => {
        if (this.state.getdata_num === true) {
            if (
                this.state.date_rank_lable.length === 0 ||
                this.state.date_rank_lable[0] === undefined
            ) {
                this.setState({
                    week_num: this.state.week.reduce((a, b) => a + b, 0),
                });
                this.setState({
                    month_num: this.state.month.reduce((a, b) => a + b, 0),
                });
                this.setState({
                    year_num: this.state.year.reduce((a, b) => a + b, 0),
                });
                this.setState({
                    lifetime_num: this.state.lifetime.reduce(
                        (a, b) => a + b,
                        0
                    ),
                });
                let today = new Date();
                this.getDataFromDbDate2(
                    new Date(
                        today.getFullYear(),
                        today.getMonth(),
                        today.getDate() - 14
                ...
                var mapper_date = this.state.allData.map(data =>
                    data.date.substring(0, 10)
                );
                var mapper_city = this.state.allData.map(
                    data => data.city + ', ' + data.state
                );
                var mapper_address = this.state.allData.map(
                    data => data.address
                );
